...
export default function PrintStockRequest({ stock_sritem, stockrequest }){
    return(
        <div className="m-5  "
        >
        <Head title={`Receiving "${stockrequest.id}" `}/>
     ...
     <div class="py-5 p-16 bg-white capitalize print:p-0">
        <div class="max-w-full mx-auto  text-gray-900 grid grid-cols-1 md:grid-cols-3 gap-2">
            <div class="overflow-hidden shadow-sm sm:rounded-lg col-span-3">
               ...
               <div className="text-right">
                ...
                <div class="mt-3 flex justify-between leading-7">
                    <div class="font-light text-xs md:text-sm">
                        <div>
                            <label for="ReceivingId">To. :</label>
                            <span className="mx-2 font-bold underline underline-offset-4">{stockrequest.sr_to}</span>
                        </div>
                    </div>
                     <div className="font-light text-xs md:text-sm">
                        <div>
                            <label for="ReceivingId"></label>
                            <span  className="mx-2"></span>
                        </div>
                        
                    </div> 
                    <div  className="font-light text-xs md:text-sm mr-32">
                        <div>
                            <label for="ReceivingId">RS No. :</label>
                            <span  className="mx-2 underline underline-offset-4">{stockrequest.rs_no}</span>
                        </div>
                        <div>
                            <label for="ReceivingId">Date :</label>
                            <span  className="mx-2 underline underline-offset-4">{stockrequest.sr_date}</span>
                        </div>
                    </div>
                </div>
        ...
        <table class="  px-20 w-full mx-auto text-sm text-left rtl:text-right border-solid border border-gray-800 text-gray-500 ">
              <thead class="text-xs text-gray-700  uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                  <tr class="text-center">
                                      ...
                                      <th class=" py-1 min-w-[120px] border border-gray-500">ITEM</th>
                                      <th class=" py-1 border border-gray-500">QTY</th>
                                      <th class=" py-1 border border-gray-500">UNIT</th>
                                      <th class=" py-1 w-full  border border-gray-500">DESCRIPTION</th>
                                  </tr>
              </thead>
              <tbody>
                         {stock_sritem.map((item, index) => (
                             <tr key={item.id} className="text-center text-xs">
        ...
        <div>
           <label for="ReceivingRemarks">Notes :</label>
             <span>{stockrequest.sr_notes}</span>
      </div>
        </div>
        <div className=" grid grid-cols-1 md:grid-cols-3 gap-4 text-xs md:text-sm md:gap-10 leading-1 md:leading-7">
            <div class="border-b py-8 border-gray-500">Prepared By:</div>
            <div class="border-b py-8 border-gray-500">Approved By:</div>
            <div class="border-b py-8 border-gray-500">Received By:</div>
        </div>
        <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-4 text-xs md:text-sm md:gap-10 leading-6 md:leading-7">
            <div class="border-b border-gray-500">Date:</div>
            <div class="border-b border-gray-500">Date:</div>
            <div class="border-b border-gray-500">Date:</div>
        </div>
