...
// force delete for items
const forcedeleteItem = (item) => {
  if(!window.confirm("Are you sure you want to delete  the Item")){
    return;
  }
  router.delete(route('item.forceDeleteItem', item.id))
}

// force delete for receivings
const submitDelete = (id) => {
  console.log(id);
  if (id === null) {
    return;
  }
  router.delete(route('receiving.forceDelete', id))
 
  setDisplayConfirmationModal(false);
  setShowModal(false);
};

// smooth scrolling of tables
...
const handleScroll = (tableId) => {
  setActiveTableId(tableId);
};

const scrollToTable = () => {
  if (activeTableId) {
    const targetTable = activeTableId === 'table1' ? table1Ref.current : table2Ref.current;
    if (targetTable) {
...
useEffect(() => {
  scrollToTable();
}, [activeTableId]); // Scroll on active table change
  ...
  return (
    ...
    >
      {/* head displayed together with the appname */}
       ...
       <div className="flex px-8 p-2 flex-row gap-4">
          <button className="flex flex-row items-center font-bold  text-blue-600"  onClick={() => handleScroll("table1")}> 
                  <Nitem className="block h-9 w-auto fill-green-500 " />
                  <span>Item</span> 
                </button>
                
                <button className="flex flex-row  items-center font-bold text-blue-600"   onClick={() => handleScroll("table2")}> 
                <Nreceiving className="block h-9 w-auto fill-current " /> 
                  <span>Receiving</span> 
                </button>
                {/* <button className="flex flex-row items-center font-bold text-blue-600" onClick={handleDR}> 
      ...
      <div className="py-8">
          ...
          <div className="max-w-5/6 mx-auto sm:px-6 lg:px-8 relative">
            ...
            {/* {success && (
              ...
              <div id="table1" ref={table1Ref} className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                              ...
                              {items.data.map((item)=>(
                                   ...
                                   <td className="w-[100px] py-2 text-nowrap">
                                       <div className="flex flex-row justify-end items-center">
                                        ...
                                        <button 
                                                  onClick={(e) =>forcedeleteItem(item)}
           ...
           <div className=" px-10 pt-12 w-full">
              ...
              {/* MRR HALF */}
                <div id="tableRef2" ref={table2Ref} className="col-span-1 w-full">
                <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div className="p-6 text-gray-900 dark:text-gray-100">
                                <div className="overflow-auto">
                                    <div className="w-full flex flex-row justify-between items-center mb-2">
                                        ...
                                        <tbody className="h-28 ">
                                            {receivings.data.map((receiving, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="w-24 text-base font-bold text-center">{receiving.mrr_no}</td>
                                                    <td className="w-28">
                                                    <p className={`whitespace-no-wrap text-center rounded-lg
                                                     `}>
                                                      ...
                                                      <Link href={route('receiving.restoreReceiving',receiving.id) } className="font-medium py-1 px-2 text-green-600 hover:bg-green-600 hover:text-white hover:rounded-full dark:text-green-500 hover:underline mx-1">
                                                        ...
                                                        <button
                                                            className="px-2 py-1 text-red-500  hover:bg-red-600 hover:text-white hover:rounded-full"
                                                            type="button"
                                                            onClick={() => {[setShowModal(true),showDeleteModal(receiving.id)]; }}
                                                        >
                                                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                                          </svg>
              ...
              {/* DELIVERABLES HALF */}
                <div ref={refDR} className="col-span-1 ">
                <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div className="p-6 text-gray-900 dark:text-gray-100">
                                <div className="overflow-auto">
                                    <div className="w-full flex flex-row justify-between items-center mb-2">
